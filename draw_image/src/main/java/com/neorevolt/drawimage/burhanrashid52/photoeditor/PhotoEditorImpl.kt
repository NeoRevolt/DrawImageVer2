...
import com.neorevolt.drawimage.burhanrashid52.photoeditor.PhotoEditor.OnSaveListener
import com.neorevolt.drawimage.burhanrashid52.photoeditor.PhotoEditorImageViewListener.OnSingleTapUpCallback
...
internal class PhotoEditorImpl @SuppressLint("ClickableViewAccessibility") constructor(
    ...
    override fun addImage(desiredImage: Bitmap?) {
        ...
        sticker.buildView(desiredImage,100, null)
        addToEditor(sticker)
    }

    override fun addImage(desiredImage: Bitmap?, inputSize: Int) {
        val multiTouchListener = getMultiTouchListener(true)
        val sticker = Sticker(photoEditorView, multiTouchListener, viewState, mGraphicManager)
    ...
    override fun editSticker(view: View?, progress: Int) {
        val seekBar = view?.findViewById<SeekBar>(R.id.sbStickerSize)
        seekBar?.progress = progress
        val stickerStyleBuilder = StickerStyleBuilder()
        stickerStyleBuilder.withStickerSize((seekBar?.progress?:0).toFloat())
        editSticker(view, seekBar?.progress ?:0, null)
    }

    override fun editSticker(view: View?, progress: Int, desiredImage: Bitmap?) {
        if (desiredImage != null){
            val seekBar = view?.findViewById<SeekBar>(R.id.sbStickerSize)
            seekBar?.progress = progress
            val stickerStyleBuilder = StickerStyleBuilder()
            stickerStyleBuilder.withStickerSize((seekBar?.progress?:0).toFloat())
            stickerStyleBuilder.applyStyle(desiredImage, (seekBar?.progress?:0).toFloat())
            if (view != null) {
                mGraphicManager.updateView(view)
            }
        }
